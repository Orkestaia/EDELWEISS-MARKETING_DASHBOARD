...
export function OrdersDashboard({ data }: { data: OrdersAndRequestsData }) {
  ...
  return <section className="orders-dashboard">
    ...
    {view === 'analytics' && <div className="product-insights">
      <div className="insight-summary"><article><BarChart3/><span>Units sold</span><strong>{analytics.totalUnits}</strong><small>real web orders</small></article><article><Sparkles/><span>All-time leader</span><strong>{analytics.allTime[0]?.name || '—'}</strong><small>{analytics.allTime[0]?.quantity || 0} units</small></article><article><CalendarClock/><span>Monthly leader</span><strong>{analytics.month[0]?.name || '—'}</strong><small>{analytics.month[0]?.quantity || 0} units in {analytics.latestDate.toLocaleString('en-US', { month: 'long' })}</small></article></div>
